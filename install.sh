...
# @@File             :  install.sh
# @@Description      :  Container installer script for traefik
...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/traefik"
APPVERSION="$(__appversion "$REPO/raw/$REPO_BRANCH/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="traefik"
export INSTDIR="$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/traefik"
export DOCKERMGR_CONFIG_DIR="${DOCKERMGR_CONFIG_DIR:-$HOME/.config/myscripts/$SCRIPTS_PREFIX}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the mountpoint directory
export APPDIR="$HOME/.local/share/srv/docker/traefik"
...
HUB_IMAGE_URL="casjaysdevdocker/traefik"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [traefik.$SET_HOST_FULL_NAME] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
if [ -n "$TRAEFIK_USERNAME" ]; then
  CONTAINER_USER_NAME="$TRAEFIK_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${TRAEFIK_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
